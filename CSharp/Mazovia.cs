...
namespace JPKFVHurt.CSharp
{
    ...
    : Encoding
    {
        ...
        public override int GetBytes(char[] chars, int charIndex, int charCount, byte[] bytes, int byteIndex)
        {
            ...
            {
                char cur = chars[i];
                if (!unicodeToByte.TryGetValue(cur, out byte asMazovia)) asMazovia = 0x003F; 
                total++;
        ...
        public override int GetChars(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex)
        {
            ...
            if (byteIndex + byteCount > bytes.Length || charIndex < 0 || byteIndex < 0 || charIndex + byteCount > chars.Length)
            {
                throw new ArgumentOutOfRangeException();
            }
        ...
        {
            if (charArray == null) throw new ArgumentNullException();

            if (index + count <= charArray.Length && index >= 0 && count >= 0) return count; 
